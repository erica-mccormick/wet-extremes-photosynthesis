...
def main():
    ...
    for file in glob.glob(paths.FLUXNET_HH_DIR + '/*'):
        ...
        if site_name in swc_sites:
            ...
            # Also find somewhat extreme days to remove from training dataset
            medium_extreme_to_remove = get_events_df(df_hh, site_name, percentile_thresh = 0.8, consec_timestep_thresh = 64, CSV_OUTPUT_DIR = CSV_OUTPUT_DIR)
            ...
            if events.shape[0] > 0:
                ...
                medium_events = get_event_days(medium_extreme_to_remove, site_name)
                ...
                if 'doy' not in medium_events.columns:
                    medium_events['doy'] = pd.to_datetime(medium_events['day']).dt.dayofyear
                ...
                # Remove medium events from hh_train
                hh_train = hh_train[~hh_train['day'].isin(medium_events['day'])]
